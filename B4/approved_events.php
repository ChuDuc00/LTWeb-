...
<head>
    ...
    <style>
        ...
        table { 
            ...
            border-radius: 12px; 
            overflow: hidden;
            table-layout: auto; /* Tự điều chỉnh kích thước thông minh theo chữ */
        }
        ...
        th { 
            ...
            color: #475569;
            font-weight: 600;
            white-space: nowrap; /* Cấm tiêu đề gãy dòng gây lệch dòng */
        }
        ...
        <tbody>
            ...
            foreach ($_SESSION['eventList'] as $item): 
                if (isset($item['status']) && $item['status'] === 'Đã duyệt'):
                    $hasApproved = true;
                    $isPast = isset($item['time']) ? (strtotime($item['time']) < time()) : false;
            ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <b>Thời gian:</b> <?= isset($item['time']) ? date('d/m/Y H:i', strtotime($item['time'])) : '' ?><br>
                            <b>Địa điểm:</b> <?= htmlspecialchars($item['location'] ?? '') ?> (<?= htmlspecialchars($item['participants'] ?? '0') ?> người)
                        </td>
            ...
            if (!$hasApproved): ?>
                <tr><td colspan="6" style="text-align:center; color: #94a3b8; padding: 25px;">Chưa có dữ liệu</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
